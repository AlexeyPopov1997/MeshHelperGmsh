...
namespace gmsh { // Top-level functions
  ...
  namespace model { // Per-model functions
    ...
    namespace mesh { // Per-model meshing functions
      ...
      // nodes, concatenated. The optional `parametricCoord' vector contains the
      ...
      // the number of nodes per element, that contains the node tags of all the
                                ...
                                const std::vector<double> & affineTransformation)
      {
        ...
        double *api_affineTransformation_; size_t api_affineTransformation_n_; vector2ptr(affineTransformation, &api_affineTransformation_, &api_affineTransformation_n_);
        gmshModelMeshSetPeriodic(dim, api_tags_, api_tags_n_, api_tagsSource_, api_tagsSource_n_, api_affineTransformation_, api_affineTransformation_n_, &ierr);
        if(ierr) throw ierr;
        gmshFree(api_tags_);
        gmshFree(api_tagsSource_);
        gmshFree(api_affineTransformation_);
      }
  ...
  namespace onelab { // ONELAB server functions
                      ...
                      const std::string & format = "json")
    {
      int ierr = 0;
      char *api_data_;
      gmshOnelabGet(&api_data_, format.c_str(), &ierr);
      if(ierr) throw ierr;
